...
{
    struct Stack_Structure *stack = (struct Stack_Structure *)malloc(sizeof(struct Stack_Structure));
    stack->max = max;
    stack->top = -1;
...
void add_element(struct Stack_Structure *stack, int item)
{
    if (isFull(stack))
    {
        return;
...
int remove_element(struct Stack_Structure *stack)
{
    if (isEmpty(stack))
    {
        return INT_MIN;
...
{
    int count, shift;
    char destination = 'C', source = 'A', temporary = 'B';
    if (limit % 2 == 0)
    {
        char x = destination;
